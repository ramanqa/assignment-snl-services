...
public class SNLTestV2 {
  ...
  public void beforeTest() throws ParseException {
	...
	RestAssured.authentication = authScheme;
	 obj = new SNLMain();
	 Response response = RestAssured.given().param("response.status", "1").when().get("http://10.0.1.86/snl/rest/v2/board/new.json");
	...
	{    
		Response response = RestAssured.when().get("http://10.0.1.86/snl/rest/v2/board/"+boardId+".json");
		response.then().assertThat().statusCode(200);
		response.then().body("response.board.id",equalTo(boardId));
		ArrayList listBoardResponse = RestAssured.when().get("http://10.0.1.86/snl/rest/v2/board.json").jsonPath().get("response.board.id");
	...
	public void testAddNewPlayer()
	{
		...
		Response response = 	RestAssured.given().contentType(ContentType.JSON).body(json).when().post("http://10.0.1.86/snl/rest/v2/player.json");
		   response.then().assertThat().statusCode(200); 
		playerId =  response.jsonPath().getInt("response.player.id");
		 RestAssured.get("http://10.0.1.86/snl/rest/v2/player/"+playerId+".json")
		 .then()
	 ...
	 * 
	 */
		JsonPath jsonPath = RestAssured.when().get("http://10.0.1.86/snl/rest/v2/board/"+boardId+".json").jsonPath();
	...
	{
		Response response = RestAssured.when().delete("http://10.0.1.86/snl/rest/v2/board/"+boardId+".json");
		response.then().assertThat().statusCode(200);
	...
	{                            //
	RestAssured.when().get("http://10.0.1.86/snl/rest/v2/player/"+playerId+".json").then()
	...
	RestAssured.given().contentType(ContentType.JSON).body(updateJson).when().put("http://10.0.1.86/snl/rest/v2/player/"+playerId+".json").then()
	...
	{
	 RestAssured.when().delete("http://10.0.1.86/snl/rest/v2/player/"+playerId+".json").then().body("response.success", equalTo("OK"))
	 .assertThat().statusCode(200);
	...
	{
	Response response = RestAssured.when().get("http://10.0.1.86/snl/rest/v2/move/"+boardId+".json?player_id="+playerId);
	response.then().assertThat().statusCode(200);
