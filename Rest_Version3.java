...
public class Rest_Version3 {
	    ...
	    String response =
	            RestAssured.given()
	                .parameters("username", "su", "password", "root_pass", 
	                           ...
	                           "client_id", "aa9398312aa357b7b4c4c1440b6c7bf9c172514fccf9705c71c258164a60eaff", "client_secret", "85abdad785b0858a2a8421f2476eeb612db9b4b0645f15c79c85bcfb90d92d45")
	                .auth()
	                .preemptive()
	                .basic("aa9398312aa357b7b4c4c1440b6c7bf9c172514fccf9705c71c258164a60eaff","85abdad785b0858a2a8421f2476eeb612db9b4b0645f15c79c85bcfb90d92d45")
	            .when()
	                .post("/oauth/token")
	                .asString();
	...
	public void Create_a_new_board() {
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().get("/rest/v3/board/new.json");
	...
	public void Get_a_list_of_Boards() {
		...
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().get("/rest/v3/board.json");
		res.then().assertThat().statusCode(200);
		JsonPath obj1=res.jsonPath();
	...
	public void Get_Board_Details()  {
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().get("/rest/v3/board/{id}", Board_id);
		res.then()
	...
	public void Put_Board_Details()  {
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().put("/rest/v3/board/{id}", Board_id);
		res.then()
		.assertThat().statusCode(200);
		JsonPath obj1= res.jsonPath();
	...
	public void Destroy_Boards() {
		...
		String id;
		int count=0;
	com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).when().delete("/rest/v3/board/{id}", Board_id);
	res.then().assertThat().statusCode(200);
	
	com.jayway.restassured.response.Response res1=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().get("/rest/v1/board.json");
...
/*	@Test
	...
	public void Join_new_Player_to_board() throws IOException {
		...
		JsonPath jsonpath=RestAssured.given().auth().oauth2(accessToken).body(ob1).when().post("/rest/v3/player.json").then().extract().jsonPath();
	...
	public void Get_Player_details() {
		...
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).param("response.status", "1").when().get("/rest/v3/player/{id}",Player_id);
		res.then().assertThat().statusCode(200);
	...
	JSONObject ob1=new JSONObject();
		...
		System.out.println(Player_id);
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).body(ob2).when().put("/rest/v3/player/{id}",Player_id);
		res.then().assertThat().statusCode(200);
...
JSONObject ob1=new JSONObject();
		...
		//System.out.println(Player_id);
		com.jayway.restassured.response.Response res=RestAssured.given().auth().oauth2(accessToken).when().get("/rest/v3/move/{id}?player_id={id1}",Board_id,Player);
		res.then().assertThat().statusCode(200);
	...
	RestAssured.given().auth().oauth2(accessToken).when().delete("/rest/v3/player/{id}", Player_id).then().assertThat().statusCode(200);
